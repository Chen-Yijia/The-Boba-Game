...
renderplotsHistoryPrice <- function(input, output, historyDemand) {
  ...
  # if (nrow(historyDemand)>30){
  #   reviseddata <- tail(historyDemand,-(nrow(historyDemand)-30))
  # } else {
  #   reviseddata <- historyDemand
  # }
  # 
  # 
  # # history plot
  ...
  # ft_label <- paste0("( ", input$ftpriceinteger, " , ", Demand_FT[which(Price_FT==input$ftpriceinteger)], " )")
  ...
  output$currentdata <- renderPlot({
    ggplot(data2,aes(x=Price_MT,y=Demand_MT))+geom_line()+labs(x="Price for milk tea",y="Demand for milk tea")+
      theme(plot.background=element_rect(fill="#FFF89A"),axis.title=element_text(size=14, face="bold"),panel.background=element_blank(),axis.line=element_line(colour="black"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),) +
      geom_vline(xintercept = input$mtpriceinteger, color = "#FF8AAE", size = 2) + geom_point(x=input$mtpriceinteger, y=Demand_MT[which(Price_MT==input$mtpriceinteger)], shape = 1, size = 8) + 
  ...
  output$currentdata2 <- renderPlot({
    ggplot(data2,aes(x=Price_FT,y=Demand_FT))+geom_line()+labs(x="Price for fruit tea",y="Demand for fruit tea")+
      theme(plot.background=element_rect(fill="#FFF89A"),axis.title=element_text(size=14, face="bold"),panel.background=element_blank(),axis.line=element_line(colour="black"),panel.grid.major = element_blank(), panel.grid.minor = element_blank(),) +
      geom_vline(xintercept = input$ftpriceinteger, color = "#FF8AAE", size = 2) + geom_point(x=input$ftpriceinteger, y=Demand_FT[which(Price_FT==input$ftpriceinteger)], shape = 1, size = 8) +
...
menuquitPopUp <- function(input) {
  ...
  observeEvent(input$quitgame, {
    shinyalert(class="quit-alert","Quit game", inputId = "quitalert",confirmButtonText = "OK", cancelButtonText = "Cancel", showCancelButton = TRUE,
               showConfirmButton = TRUE, type="error", html=T, text="Progress in this level will be lost. Are you sure you want to quit?")
  })
  
  observeEvent(input$restart, {
    shinyalert(class="restart-alert","Restart game", inputId = "restartalert",confirmButtonText = "OK", cancelButtonText = "Cancel", showCancelButton = TRUE,
